 ...
 * @return {Array<{content: string, chunkIndex: number, overlap: number, wordCount: number}>}
 */
...
if (currentWordCount + paragraphWordCount > chunkSize && currentChunk.length > 0) {
  ...
  const overlapWords = prevChunkWords.slice(-Math.min(overlap, prevWords.length)).join(' ');
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  if(!chunks || chunkText.length === 0 || !query) {
    return [];
  }
...
return {
  ...
  rawScore: score,
  matchedWords: queryWords.filter(word => content.includes(word)).length
};
